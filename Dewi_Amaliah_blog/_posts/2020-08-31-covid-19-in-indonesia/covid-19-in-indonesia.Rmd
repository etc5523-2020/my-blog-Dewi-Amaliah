...
For the lockdown variable in the dataset, that I previously wanted to use, it is apparently did not match with the date of "New Normal" policy began in Indonesia. Thus, I ended up not using this data. 
...
The fact is this claim is might be incorrect, at least according to the WHO criteria. According to the WHO in May, one of the indicator that the epidemic is under control is that positive rate is less than 5% [@testing]. As per date of this statement, the positive rate in Indonesia is `r positiverate19aug` percent.


## Conclusion

Three out of four of Luhut's claims, which were checked with data from various sources, are correct, in numerical terms. However, if we look at the bigger picture and the context, these claims are actually busted. For example, if the trend is seen in its entirety since the beginning of the pandemic, the cases and the spread of Covid-19 in Indonesia cannot be said as declined. Besides, contextually, the usage of recovered cases as an indicator to claim the success of Covid-19 handling can not be justified. 
